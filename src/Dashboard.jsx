...
import { fetchAllData, fetchLogbookByAthlete } from './api';
...
export default function Dashboard() {
  ...
  const [athleteRow, setAthleteRow] = useState(-1);
    ...
    const loadDashboard = async () => {
      try {
        ...
        for (let c = 0; c < headers.length; c++) {
          ...
          if (h === 'email' || h === 'e-mail' || h === 'user email') {
            emailCol = c;
          }
        }
        ...
        let foundRow = -1;
        if (emailCol >= 0) {
          for (let r = 1; r < data.athletes.length; r++) {
            if (data.athletes[r][emailCol]) {
              if (String(data.athletes[r][emailCol]).trim().toLowerCase() === user.email.toLowerCase()) {
                foundRow = r;
                break;
              }
            }
        ...
        if (foundRow < 0) {
          for (let r = 1; r < data.athletes.length; r++) {
            if (data.athletes[r][nameCol]) {
              if (String(data.athletes[r][nameCol]).trim().toLowerCase() === user.email.toLowerCase()) {
                foundRow = r;
                break;
              }
            }
        ...
        setAthleteRow(foundRow);
        const name = String(data.athletes[foundRow][0]).trim();
        setAthleteName(name);
        ...
        if (logbookEntries.length > 0) {
          const history = logbookEntries.map(entry => ({
            date: entry.date,
            prog: entry.prog,
            ex: entry.ex,
            wt: entry.wt,
          ...
          setMyHistory(history.reverse());

          const uniqueEx = [...new Set(history.map(h => h.ex))].filter(Boolean).sort();
