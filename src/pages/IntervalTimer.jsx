...
// Audio URLs
const SOUNDS = {
  work: 'https://assets.mixkit.co/active_storage/sfx/995/995-preview.mp3', // Loud Boxing Bell
  rest: 'https://assets.mixkit.co/active_storage/sfx/2869/2869-preview.mp3', // Sharp 3-tone Beep
  finish: 'https://assets.mixkit.co/active_storage/sfx/1435/1435-preview.mp3' // Success Chime
};
...
export default function IntervalTimer() {
  ...
  const playSound = (type) => {
    ...
    try {
      ...
      if (audio) {
        ...
        // FIX: Sniper cutoff for the Work Bell (stops after 3.5s)
        if (type === 'work') {
          setTimeout(() => {
            audio.pause();
            audio.currentTime = 0;
  ...
  // FIX: Smarter +1 Round logic
  const addOneRound = () => {
    if (activeSequence.length === 0) return;
    
    // Auto-detect a "Round" as the sequence up to the first 'rest'
    let roundEndIdx = activeSequence.findIndex(s => s.type === 'rest');
    if (roundEndIdx === -1) roundEndIdx = activeSequence.length - 1; 
    
    const baseRound = activeSequence.slice(0, roundEndIdx + 1).map(step => ({
      ...step,
      id: Date.now() + Math.random()
    }));
    
    setActiveSequence([...activeSequence, ...baseRound]);
  };
      ...
      <div style={{ backgroundColor: bgColor, minHeight: '100vh', display: 'flex', flexDirection: 'column', color: 'white', fontFamily: '"SF Pro Display", sans-serif', transition: 'background-color 0.4s' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '20px' }}>
          ...
          {isLooping && !isFinished && (
            <div style={{ fontSize: '56px', fontWeight: '900', letterSpacing: '4px', margin: '0 0 24px 0', padding: '12px 40px', backgroundColor: 'rgba(255, 255, 0, 0.2)', borderRadius: '24px', textShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
              ROUND {loopCount}
            </div>
          )}
          ...
          {!isFinished && (
            <div style={{ marginTop: '60px', display: 'flex', gap: '20px' }}>
              <button onClick={() => setTimerState(timerState === 'running' ? 'paused' : 'running')} style={{ backgroundColor: 'white', color: bgColor, border: 'none', borderRadius: '50px', padding: '20px 40px', fontSize: '24px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px', boxShadow: '0 8px 24px rgba(0,0,0,0.2)' }}>
  ...
  return (
    <div style={{ padding: '16px', maxWidth: '600px', margin: '0 auto', fontFamily: '"Roboto Flex", sans-serif', backgroundColor: '#f8fafc', minHeight: '100vh', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '24px' }}>
        ...
        <h2 style={{ fontSize: '24px', color: '#0f172a', fontWeight: '700', margin: 0 }}>Interval Timer</h2>
      </div>
      ...
      {presets.length > 0 && (
        <div style={{ backgroundColor: 'white', padding: '16px', borderRadius: '12px', marginBottom: '24px', border: '1px solid #e2e8f0' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <select value={selectedPresetId} onChange={(e) => loadPreset(e.target.value)} style={{ flex: 1, padding: '10px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '15px' }}>
              <option value="">-- Create New Timer --</option>
              {presets.map(p => <option key={p.id} value={p.id}>{p.name} ({getTotalTime(p.sequence)}) {p.isLooping ? '🔁' : ''}</option>)}
            </select>
            {selectedPresetId && <button onClick={() => deletePreset(selectedPresetId)} style={{ background: '#fef2f2', color: '#ef4444', border: '1px solid #fca5a5', padding: '10px', borderRadius: '8px', cursor: 'pointer' }}><Trash2 size={18} /></button>}
          </div>
        </div>
      )}
      ...
      <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '12px', marginBottom: '24px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', marginBottom: '20px' }}>
          <div style={{ flex: 1 }}><label style={{ display: 'block', fontSize: '12px', color: '#64748b', marginBottom: '4px' }}>Minutes</label><input type="number" min="0" value={inputMin} onChange={e => setInputMin(e.target.value)} placeholder="0" style={{ width: '100%', padding: '12px', fontSize: '20px', textAlign: 'center', borderRadius: '8px', border: '2px solid #e2e8f0' }} /></div>
          <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#cbd5e1', alignSelf: 'center', marginTop: '16px' }}>:</div>
          <div style={{ flex: 1 }}><label style={{ display: 'block', fontSize: '12px', color: '#64748b', marginBottom: '4px' }}>Seconds</label><input type="number" min="0" max="59" value={inputSec} onChange={e => setInputSec(e.target.value)} placeholder="0" style={{ width: '100%', padding: '12px', fontSize: '20px', textAlign: 'center', borderRadius: '8px', border: '2px solid #e2e8f0' }} /></div>
        </div>
        <div style={{ display: 'flex', gap: '12px' }}>
          <button onClick={() => addStep('work')} style={{ flex: 1, backgroundColor: '#e0f2fe', color: '#0284c7', border: 'none', padding: '14px', borderRadius: '8px', fontSize: '15px', fontWeight: '700', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px' }}><Activity size={18} /> Add WORK</button>
          <button onClick={() => addStep('rest')} style={{ flex: 1, backgroundColor: '#ffedd5', color: '#d97706', border: 'none', padding: '14px', borderRadius: '8px', fontSize: '15px', fontWeight: '700', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px' }}><Coffee size={18} /> Add REST</button>
        </div>
      </div>
      ...
      <div style={{ marginBottom: '32px' }}>
        ...
        {activeSequence.length === 0 ? <div style={{ textAlign: 'center', padding: '32px', backgroundColor: 'white', borderRadius: '12px', border: '1px dashed #cbd5e1', color: '#94a3b8' }}>No intervals added yet.</div> : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {activeSequence.map((step, index) => (
              <div key={step.id} style={{ display: 'flex', alignItems: 'center', padding: '12px 16px', backgroundColor: 'white', borderRadius: '8px', borderLeft: `4px solid ${step.type === 'work' ? '#0ea5e9' : '#f59e0b'}`, boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
                ...
                <button onClick={() => removeStep(step.id)} style={{ background: 'none', border: 'none', color: '#ef4444', padding: '8px', marginLeft: '12px', cursor: 'pointer' }}><Trash2 size={18} /></button>
              </div>
            ))}
            <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
              <button onClick={addOneRound} style={{ flex: 1, padding: '12px', backgroundColor: '#f1f5f9', border: '1px dashed #cbd5e1', borderRadius: '8px', color: '#475569', fontWeight: '600', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', cursor: 'pointer' }}><PlusCircle size={16} /> +1 Round</button>
              <button onClick={duplicateSequence} style={{ flex: 1, padding: '12px', backgroundColor: '#f1f5f9', border: '1px dashed #cbd5e1', borderRadius: '8px', color: '#475569', fontWeight: '600', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', cursor: 'pointer' }}><Copy size={16} /> x2 Duplicate All</button>
            </div>
          </div>
        )}
      </div>
      ...
      {activeSequence.length > 0 && (
        <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
          <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
            <input type="text" value={presetName} onChange={e => setPresetName(e.target.value)} placeholder="Name to save preset..." style={{ flex: 1, padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '15px' }} />
            <button onClick={savePreset} style={{ backgroundColor: '#f1f5f9', color: '#475569', border: '1px solid #cbd5e1', padding: '0 16px', borderRadius: '8px', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}><Save size={18} /> Save</button>
          </div>
          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '12px', backgroundColor: isLooping ? '#e0f2fe' : '#f8fafc', border: `1px solid ${isLooping ? '#bae6fd' : '#e2e8f0'}`, borderRadius: '8px', marginBottom: '24px', cursor: 'pointer', transition: 'all 0.2s' }}>
            <input type="checkbox" checked={isLooping} onChange={(e) => setIsLooping(e.target.checked)} style={{ width: '18px', height: '18px', cursor: 'pointer' }} />
            <span style={{ fontSize: '15px', fontWeight: '600', color: isLooping ? '#0284c7' : '#475569', display: 'flex', alignItems: 'center', gap: '6px' }}><Repeat size={18} /> Loop Continuously</span>
          </label>
          <button onClick={startTimer} style={{ width: '100%', backgroundColor: '#10b981', color: 'white', border: 'none', padding: '18px', borderRadius: '12px', fontSize: '20px', fontWeight: '800', textTransform: 'uppercase', letterSpacing: '1px', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '12px', boxShadow: '0 4px 12px rgba(16, 185, 129, 0.3)' }}><Play size={24} fill="currentColor" /> Start Workout</button>
        </div>
      )}
