...
function ConeMarker(props) {
  return (
    <Group x={props.x} y={props.y}>
      <Circle radius={18} fill={props.coneColor} stroke="#000" strokeWidth={2} />
      <Circle radius={12} fill="#fff" stroke={props.coneColor} strokeWidth={1} />
      <Circle radius={6} fill={props.coneColor} />
    </Group>
  );
}
...
function FootballFieldLines() {
  return (
    <Layer listening={false}>
      ...
      <Rect x={10} y={10} width={CANVAS_WIDTH - 20} height={CANVAS_HEIGHT - 20} stroke="white" strokeWidth={3} fillEnabled={false} />
      <Line points={[CANVAS_WIDTH / 2, 10, CANVAS_WIDTH / 2, CANVAS_HEIGHT - 10]} stroke="white" strokeWidth={3} />
      <Circle x={CANVAS_WIDTH / 2} y={CANVAS_HEIGHT / 2} radius={CANVAS_HEIGHT * 0.15} stroke="white" strokeWidth={3} fillEnabled={false} />
      <Rect x={10} y={CANVAS_HEIGHT / 2 - CANVAS_HEIGHT * 0.15} width={CANVAS_WIDTH * 0.12} height={CANVAS_HEIGHT * 0.3} stroke="white" strokeWidth={2} fillEnabled={false} />
      <Rect x={CANVAS_WIDTH - 10 - CANVAS_WIDTH * 0.12} y={CANVAS_HEIGHT / 2 - CANVAS_HEIGHT * 0.15} width={CANVAS_WIDTH * 0.12} height={CANVAS_HEIGHT * 0.3} stroke="white" strokeWidth={2} fillEnabled={false} />
    </Layer>
  );
}
...
function BasketballCourtLines() {
  return (
    <Layer listening={false}>
      ...
      <Rect x={10} y={10} width={CANVAS_WIDTH - 20} height={CANVAS_HEIGHT - 20} stroke="#333" strokeWidth={3} fillEnabled={false} />
      <Line points={[CANVAS_WIDTH / 2, 10, CANVAS_WIDTH / 2, CANVAS_HEIGHT - 10]} stroke="#333" strokeWidth={3} />
      <Circle x={CANVAS_WIDTH / 2} y={CANVAS_HEIGHT / 2} radius={45} stroke="#333" strokeWidth={3} fillEnabled={false} />
      <Rect x={10} y={CANVAS_HEIGHT / 2 - 85} width={CANVAS_WIDTH * 0.15} height={170} stroke="#333" strokeWidth={2} fillEnabled={false} />
      <Rect x={CANVAS_WIDTH - 10 - CANVAS_WIDTH * 0.15} y={CANVAS_HEIGHT / 2 - 85} width={CANVAS_WIDTH * 0.15} height={170} stroke="#333" strokeWidth={2} fillEnabled={false} />
    </Layer>
  );
}
...
function BlankCanvasBorder() {
  return (
    <Layer listening={false}>
      ...
      <Rect x={0} y={0} width={CANVAS_WIDTH} height={CANVAS_HEIGHT} stroke="#ddd" strokeWidth={2} fillEnabled={false} />
    </Layer>
  );
}
...
function Whiteboard() {
  ...
  const handleMouseDown = (e) => {
    ...
    if (tool === 'text') { 
      ...
      if (newText) { 
        shapesRef.current = [...shapesRef.current, { id, type: 'text', x: pos.x, y: pos.y, text: newText, fontSize: 18, fontFamily: '"Roboto Flex", sans-serif', fill: strokeColor, fontStyle: 'bold' }]; 
        addToHistory(); 
      } 
    } else if (tool === 'player') { 
      shapesRef.current = [...shapesRef.current, { id, type: 'player', x: pos.x, y: pos.y, teamColor, number: playerNumber }]; 
      addToHistory(); 
    } else if (tool === 'cone') { 
      shapesRef.current = [...shapesRef.current, { id, type: 'cone', x: pos.x, y: pos.y, coneColor }]; 
      addToHistory(); 
    } else { 
      isDrawingRef.current = true; 
      const newShape = { id, type: tool, stroke: strokeColor, strokeWidth, lineCap: 'round' }; 
  ...
  const handleMouseMove = (e) => {
    ...
    if (current.type === 'line' || current.type === 'arrow') {
      updated.points = [current.points[0], current.points[1], pos.x, pos.y];
    } else if (current.type === 'rect') { 
      updated.width = pos.x - current.x; 
      updated.height = pos.y - current.y; 
    } else if (current.type === 'circle') {
      updated.radius = Math.max(1, Math.sqrt(Math.pow(pos.x - current.x, 2) + Math.pow(pos.y - current.y, 2)));
    }
  ...
  const handleTemplateChange = (newTemplate) => { 
    if (shapesRef.current.length > 0 && !window.confirm('Changing templates will clear drawings. Continue?')) return; 
  ...
  const getFieldLines = () => { 
    if (currentTemplate === 'football') return <FootballFieldLines />; 
    if (currentTemplate === 'basketball') return <BasketballCourtLines />; 
    return <BlankCanvasBorder />; 
  };
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '15px' }}>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '5px' }}>
              {DRAWING_TOOLS.map((t) => (
                <button key={t} onClick={() => setTool(t)} style={{ padding: '8px 4px', border: tool === t ? `2px solid ${DESIGN.primaryBlue}` : `1px solid #ddd`, borderRadius: '4px', backgroundColor: tool === t ? DESIGN.primaryBlue : '#fff', color: tool === t ? '#fff' : DESIGN.darkText, cursor: 'pointer', fontSize: '10px', textTransform: 'uppercase', fontWeight: '600', minHeight: '32px' }}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
              {['#000', '#dc2626', '#2563eb', '#eab308', '#22c55e', '#a855f7', '#ffffff'].map((color) => (
                <button key={color} onClick={() => setStrokeColor(color)} style={{ width: '32px', height: '32px', border: strokeColor === color ? `2px solid ${DESIGN.primaryBlue}` : '1px solid #ddd', borderRadius: '50%', backgroundColor: color, cursor: 'pointer' }} />
              ))}
            </div>
          </div>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
              {TEAM_COLORS.map((tc) => (
                <button key={tc.name} onClick={() => setTeamColor(tc.hex)} style={{ width: '32px', height: '32px', border: teamColor === tc.hex ? `2px solid ${DESIGN.primaryBlue}` : '1px solid #ddd', borderRadius: '4px', backgroundColor: tc.hex, cursor: 'pointer', fontSize: '10px', color: '#fff', fontWeight: 'bold' }}>
                  {tc.name[0]}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
              {CONE_COLORS.map((color) => (
                <button key={color} onClick={() => setConeColor(color)} style={{ width: '32px', height: '32px', border: coneColor === color ? `2px solid ${DESIGN.primaryBlue}` : '1px solid #ddd', borderRadius: '4px', backgroundColor: color, cursor: 'pointer' }} />
              ))}
            </div>
          </div>
        </div>
      ...
      {showSaveModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ backgroundColor: '#fff', borderRadius: '8px', padding: '25px', minWidth: '400px', maxWidth: '90vw', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            {error && (
              <div style={{ padding: '10px', backgroundColor: '#fee', borderRadius: '4px', marginBottom: '15px', color: '#c00' }}>{error}</div>
            )}
